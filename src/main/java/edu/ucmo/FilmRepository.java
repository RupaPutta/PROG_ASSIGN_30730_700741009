...
public interface FilmRepository extends Neo4jRepository<Film, Long> {
    ...
    @Query("MATCH (film:Film)-[:ACTED_IN]-(actor:Actor) RETURN film.film_id AS film_id, film.title AS title, COLLECT(actor.list_actors) AS actors")
    List<QueryResultObj> getFilmAndShowDetailsWithActorNames();
}
